const RMGantt = {
    ...
    openGap(event,room,timeline){if(event.target!==timeline||!room.active)return;const x=event.clientX-timeline.getBoundingClientRect().left,value=this.isoDate(this.dateAtPosition(x));if(value<this.data.window.today)return;BookingUI.openCreateModal(room.id,value);BookingUI.modal.show();},
